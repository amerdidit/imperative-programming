...
* In the extended version, the statement following the `then` *reserved word* is executed if the *condition* evaluates to `false`.
...
begin
    ...
    begin
        ...
        if ammount > 10 then
            ...
            if ammount > 50 then
                { big discount }
                price := price * 0.8Ì∞ç 0.8
            else
                { small discount }
                price := price * 0.9;Ì∞ç 0.9;
